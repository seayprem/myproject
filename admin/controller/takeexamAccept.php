...
if(isset($_GET['id'])) {
	$id = $_GET['id'];
	$get_sql = "SELECT * FROM sent_exam WHERE sent_checked = '".$id."'";
	$get_query = mysqli_query($conn, $get_sql);
	$get_row = mysqli_fetch_assoc($get_query);
	$sent_no = $get_row['sent_no'];
	$officer_id = $_POST['officer_id'];
	$sql = "UPDATE sent_exam SET sent_checked = '".$id."'";
	$query = mysqli_query($conn, $sql);
	if($query) {
		$insert_sql = "INSERT INTO take_exam (take_date, officer_id, sent_no) VALUES ('".$idk."', '".$officer_id."', '".$sent_no."')";
